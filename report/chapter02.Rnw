...
As we can see in Figure \ref{fig:2_12}, the histogram of these two versions do not overlap and therefore we can empirically conclude that these ways of simulating the Poisson-Gamma model are not equivalent. Both methods share the same expectation but the spread differs. 
