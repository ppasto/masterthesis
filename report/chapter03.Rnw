...
Time & $T(c)\sim\textrm{G}(\alpha, \beta)$& $\textrm{P}(T(c)\geq 548)$ & \Sexpr{round(mean(time_erlang>548),3)} & \Sexpr{round(1-pgamma(548, shape = alpha, rate = lambda),3)}\\
Time & $T(c)\sim\textrm{Gg}(\alpha, \beta, c)$ & $\textrm{P}(T(c)\geq 548)$ & \Sexpr{round(mean(timegg_easier>548),3)} & \Sexpr{round(1-pgammagamma(548, alpha = alpha, b = beta, c = 324),3)}
\end{tabular}
