...
\hline
\hline
Expectation & $T = C/\lambda$ & $C/\lambda$ & 0 & No & No \\
Erlang & $T \sim \textrm{G}(C, \lambda) $ & $C/\lambda$ & $C/\lambda^2$ & Yes & No \\
Gamma-Gamma & $T \sim \textrm{G}(C, \Lambda)$; $\Lambda \sim \textrm{G}(\alpha,\beta)$ & $C\frac{\beta}{\alpha-1}$ & $\frac{C\beta^2(C+\alpha-1)}{(\alpha-1)^2(\alpha-2)}$ & Yes & Yes \\
\end{tabular}
}
\caption{Moments and aleatory and epistemic uncertainty of recruitment covered by different models for time}
