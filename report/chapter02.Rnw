...
One way to incorporate variation in the mean number of participants per month is by assuming that participants enroll according to a known probability distribution, such as the Poisson distribution \citep{carter2004application}. This approach simulates trial accrual using a random number generator and records the time required to reach the target sample size over multiple iterations, \cite{carter2005practical}. 
