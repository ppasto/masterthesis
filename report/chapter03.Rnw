...
\hline
\hline
 $C(T)\sim\textrm{Po}(\lambda T)$ & $\textrm{P}(C(T)\geq 324) = \Sexpr{round(mean(final_counts_p>324),3)}$ & \Sexpr{round(sqrt(mean(final_counts_p>324)*(1-mean(final_counts_p>324))/10^5), 3)} & \Sexpr{round(1-ppois(324, lambda*550),3)} \\
...
\hline
\hline
 $T(N)\sim\textrm{G}(N, \lambda)$& $\textrm{P}(T(N)\geq 548) = \Sexpr{round(mean(time_erlang>548),3)}$ & \Sexpr{round(sqrt(mean(time_erlang>548)*(1-mean(time_erlang>548))/M), 3)} & \Sexpr{round(1-pgamma(548, shape = 324, rate = lambda),3)}\\
$T(N)\sim\textrm{GG}(N, \alpha, \beta)$ & $\textrm{P}(T(N)\geq 548) = \Sexpr{round(mean(timegg_easier>548),3)}$ & \Sexpr{round(sqrt(mean(timegg_easier>548)*(1-mean(timegg_easier>548))/M), 3)} & \Sexpr{round(1-pgammagamma(548, alpha = alpha, b = beta, c = 324),3)}
\end{tabular}
...
Small number of simulations (up to $M=10^3$) from \cite{carter2004application} gives him inaccurate estimate of days (580), when the exact using the Erlang distribution gives  \Sexpr{round(qgamma(p = 0.9, shape = 324, rate = 0.591), 0)} to have 90\% chance of accruing $N=324$ subjects with a recruitment rate of $\lambda=0.591$.

Moreover, if we wish to be more realistic in the uncertainty our model takes into consideration and choose the Gamma-Gamma distribution, \Sexpr{round(qgammagamma(p = 0.9, alpha = 32.4, b = 54.8, c = 324), 0)} days would be required to have a 90\% chance of accruing the desired sample size. 
